...
#docker buildx build --platform linux/arm64,linux/arm/v7  --target base --push -t wxdockerhub/ubuntu-arm-base -f Dockerfile .

# Full Arm NN SDK build
#docker buildx build --platform linux/arm64,linux/arm/v7 --target sdk --push -t wxdockerhub/armnn-sdk -f Dockerfile .

# Builds the deverloper image used to create a software application with Arm NN
docker buildx build --platform linux/arm64,linux/arm/v7 --target dev --push -t wxdockerhub/armnn-dev -f Dockerfile .

# Build the release image used to run the application without the source code
docker buildx build --platform linux/arm64,linux/arm/v7 --target rel --push -t wxdockerhub/armnn-rel -f Dockerfile .
